...
static Double_t proj_mass = 31.999110139 * 931.494;
static Double_t target_mass = 2.01410177812 * 931.494;
static Double_t recoil_mass = 32.020011026 * 931.494;
static Double_t he2_mass = 2.0 * proton_mass;
static Double_t Ekin_proj = 86.* 31.999110139;//86
// static Double_t decay_frag_mass = 31.013146656*931.494 / 1000; // GeV/c^2
static Double_t decay_frag_mass = 32.020011026*931.494 / 1000; // GeV/c^2
...
// Have to be adapted to map files names
// void SetMapList(std::vector<TString> &mapList, std::vector<Double_t> &mapDist)
// {
//    for (size_t iMap = 0; iMap < 16; iMap++)
//    {
//       TString invMapFile;
//       Int_t idMap = iMap - 5;
//       if (idMap < 0)
//       {
//          invMapFile = TString::Format("/projects/ceclub/giraud/git/ATTPCROOTv2/macro/Unpack_HDF5/e18008_S800/invMap/invmap_14N/invmap_-%02d.inv", abs(idMap)); 
//       }
//       else 
//       {
//          invMapFile = TString::Format("/projects/ceclub/giraud/git/ATTPCROOTv2/macro/Unpack_HDF5/e18008_S800/invMap/invmap_14N/invmap_%02d.inv", idMap); 
//       }
//       mapList.push_back(invMapFile);
//       mapDist.push_back(-0.5+0.1*(iMap));
//       std::cout<<"SetMapList - idMap, files, dist: "<<idMap<<" "<<invMapFile<<" "<<mapDist.at(iMap)<<std::endl;
//    }
// }

// std::vector <double> GetMapVars(TInverseMap *inv_map, double x0, double y0, double afp, double bfp, double z)
// {
//    int order;
...
//    std::cout<<"inv map inpt after ata "<<x0<<" "<<afp<<" "<<y0<<" "<<bfp<<" "<<std::endl;
...
void ana_d2He_32Mg_invMap()
{
   ...
   TString digiFileName =
      "/mnt/analysis/e18008/rootAna/giraud/simulation/digi/attpcdigi_d2He_5000_run0_Ex2_32Mg.root";
   ...
   Double_t Eje_ene=-999, Eje_ata = -999, Eje_bta = -999, Eje_dta = -999;
   Double_t brho = -999;
   ...
   anatree->Branch("Eje_ene", &Eje_ene);
   ...
   SetMapList(mapList, mapDist);
   TInverseMap *invMap = new TInverseMap();
   invMap->SetDistPivotTarget(mapDist);
   invMap->ReadMultiMapFile(mapList);
   ...
   for (Int_t i = 0; i < nEvents; i++) {
      ...
      for (Int_t ipoint = 0; ipoint < Npoints; ipoint++) {
         ...
         // std::cout<<"point "<<point->GetPxOut()<<" "<<point->GetPyOut()<<" "<<point->GetPzOut()<<std::endl;

         if (trackID == 1 && VolName == "drift_volume") { // beam-like 1; decay like 4
            // Anejec = point->GetAIni();
            // Eeje = point->GetEIni();

            Aeje = point ->GetMassNum();
            Zeje = point ->GetAtomicNum();
            ...
            if (point->GetZ() >= 100) {
               ...
               Eje_ene = Eeje;
               Eje_dta = (Eeje - 2.18) / 2753.20 - 1.0; // 2753.20: 32Na 3.97Tm // 2666.61: 31Na 3.85Tm
               // Eje_dta = (Eeje-2.18)/1385.54- 1.0;//13N
               // Eje_dta = (Eeje-2.18)/1065.94- 1.0;//10B
               momEje.SetXYZ(point->GetPxOut(), point->GetPyOut(), point->GetPzOut());
      ...
      //--------!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!
      //--------!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!
      // Just for brho stuff, remove 3 following lines in order to get through d,2He analysis
      // ivt = i;
      // anatree->Fill();
      // continue; 
      //--------!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!-!
      ...
      if (patternEvent) {
         ...
         for (size_t itv = 0; itv < tv.size(); itv++) {
            ...
            // S800 inverse map ---------------------
            Double_t S800_x0 = 100.;
            Double_t S800_y0 = 50.;
            Double_t S800_afp = 0.01;
            Double_t S800_bfp = -0.01;
            // std::vector <double> S800_invMapOut = GetMapVars(invMap,S800_x0,S800_y0,S800_afp,S800_bfp,1.066-vertexMean.Z()/1000);
				
            Int_t order = 5;
            Double_t bta_corr = 1.;
            Double_t zta = 1.066-vertexMean.Z()/1000;

            Double_t S800_ata = invMap->Ata(order, S800_x0, S800_afp, S800_y0, S800_bfp, zta);
				Double_t S800_bta = invMap->Bta(order, S800_x0, S800_afp, S800_y0, S800_bfp, zta) * bta_corr;
				Double_t S800_yta = invMap->Yta(order, S800_x0, S800_afp, S800_y0, S800_bfp, zta) * 1000.;
				Double_t S800_dta = invMap->Dta(order, S800_x0, S800_afp, S800_y0, S800_bfp, zta);
				Double_t S800_thetaLab= atan(sqrt(pow(tan(S800_ata), 2) + pow(tan(S800_bta), 2)));
				Double_t S800_phi= atan(tan(S800_bta)/tan(S800_ata));
            if (S800_ata < 0)
            S800_phi = 3.141592653 + S800_phi;
            else if (S800_bta < 0)
            S800_phi = 2*3.141592653 + S800_phi;

            std::cout<<"invMap results, ata, bta, yta, dta, thetaLab, phi: "<<S800_ata<<" "<<S800_bta<<" "<<S800_yta
            <<" "<<S800_dta<<" "<<S800_thetaLab<<" "<<S800_phi<<" "<<std::endl;
