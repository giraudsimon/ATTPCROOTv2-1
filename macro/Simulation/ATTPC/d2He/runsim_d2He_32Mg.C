...
void runsim_d2He_32Mg(Int_t runNumber = 0, Double_t ExEje = 0, Int_t nEvents = 10, TString mcEngine = "TGeant4")
{
   ...
   // Output file name
   TString outFile = Form("/mnt/analysis/e18008/rootAna/giraud/simulation/g4/attpcsim_d2He_run%d_Ex%d_32Mg.root",
                          runNumber, (Int_t)ExEje);

   // Parameter file name
   TString parFile = Form("/mnt/analysis/e18008/rootAna/giraud/simulation/g4/attpcpar_d2He_run%d_Ex%d_32Mg.root",
                          runNumber, (Int_t)ExEje);
   ...
   Double_t Bmass = 31.999110139*931.494/1000.0; // Mass in GeV
   Double_t NomEnergy = 0.0053; // Nominal Energy of the beam: Only used for cross section calculation (Tracking
  ...
  //  ionGen->SetBeamEmittance(fwhmFocus,angularDiv,zFocus,rHole);
   ...
   //--- Scattered -----
    Zp.push_back(11); // 32Na
    Ap.push_back(32); //
    Qp.push_back(0);
    Pxp.push_back(0.0);
    Pyp.push_back(0.0);
    Pzp.push_back(0.0);
    Mass.push_back(32.020011026);//
    ExE.push_back(ExEje);
  ...
  // ---- Recoil -----
   ...
   // should be a reaction product (its momentum is set in the reaction generator)
    zB=11; // 32Na
    aB=32;
    massDecayB=32.020011026;

   // ---- Products ----
   // as many first indexes (zDecay.at(0)...) as the value TotDecayCases
   // Case 1
    SepEne.push_back(1.7);
    zDecay.at(0).push_back(11); // 31Na
    aDecay.at(0).push_back(31);
    qDecay.at(0).push_back(0);
    massDecay.at(0).push_back(31.013146656);
