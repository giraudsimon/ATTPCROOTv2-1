...
void runsim_d2He_33Al(Int_t runNumber = 0, Double_t ExEje = 0, Int_t nEvents = 10, TString mcEngine = "TGeant4")
{
   ...
   // Output file name
   TString outFile = Form("/mnt/analysis/e18008/rootAna/giraud/simulation/g4/attpcsim_d2He_run%d_Ex%d_33Al_decay.root",
                          runNumber, (Int_t)ExEje);

   // Parameter file name
   TString parFile = Form("/mnt/analysis/e18008/rootAna/giraud/simulation/g4/attpcpar_d2He_run%d_Ex%d_33Al_decay.root",
                          runNumber, (Int_t)ExEje);
   ...
   // Beam Information
   Int_t z = 13;  // Atomic number
   Int_t a = 33; // Mass number
   ...
   Double_t Bmass = 32.990877687*931.494/1000.0; // Mass in GeV
   Double_t NomEnergy = 0.006; // Nominal Energy of the beam: Only used for cross section calculation (Tracking
   ...
   // set the following two variables to zero if do not want beam spot
   Double_t fwhmFocus = 0.5;                          // 0.5//cm, FWHM of the gaussian distribution at beam spot
   Double_t angularDiv = 15.E-3;                      // ex:1.E-3 (rad), angular divergence of the beam
   Double_t zFocus = 0.5;                             // cm, z position (beam direction) of the beam spot
   Double_t rHole = 1.5;                              // cm, hole radius in the pad plane (entrance)
   Double_t momAcc = 0.007;                          // percentage, beam momentum acceptance
   Double_t beamAx = 0 * TMath::RadToDeg(); // deg, beam angle x direction
   Double_t beamAy = 0 * TMath::RadToDeg(); // deg, beam angle y direction
   Double_t beamOx = 0;                     // cm, beam offset at entrance window x direction
   Double_t beamOy = 0;                    // cm, beam offset at entrance window y direction
  ...
  //  ionGen->SetBeamEmittance(fwhmFocus,angularDiv,zFocus,rHole);
   ...
   //--- Scattered -----
    Zp.push_back(12); // 33Mg
    Ap.push_back(33); //
    Qp.push_back(0);
    Pxp.push_back(0.0);
    Pyp.push_back(0.0);
    Pzp.push_back(0.0);
    Mass.push_back(33.005327245);//
    ExE.push_back(ExEje);
  ...
  // ---- Recoil -----
   ...
   string filename = "cs_accba/32Mg/all2_32Mg_reduced.dat";
   ifstream inputfile;
   ...
   // should be a reaction product (its momentum is set in the reaction generator)
    zB=12; // 33Mg
    aB=33;
    massDecayB=33.005327245;

   // ---- Products ----
   // as many first indexes (zDecay.at(0)...) as the value TotDecayCases
   // Case 1
    SepEne.push_back(2.28);
    zDecay.at(0).push_back(12); // 32Mg
    aDecay.at(0).push_back(32);
    qDecay.at(0).push_back(0);
    massDecay.at(0).push_back(31.999110139);

    zDecay.at(0).push_back(0);
    aDecay.at(0).push_back(1);
    qDecay.at(0).push_back(0);
  // massDecay.at(0).push_back(1.0078250322);//proton
    massDecay.at(0).push_back(1.0086649158);//neutron
